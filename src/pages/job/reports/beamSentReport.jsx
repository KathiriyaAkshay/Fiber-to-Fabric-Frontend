...
const BeamSentReport = () => {
    ...
    const {
        data: beamSentReport,
        isLoading: isBeamSentReportLoading
    } = useQuery({
        queryKey: ["beamSentReport/list", { company_id: companyId }],
        queryFn: async () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                let current_sent_meter = Number(record?.sent_meter);
                let current_received_meter = Number(record?.received_meter);
                let shortage = Number(current_received_meter) * 100 / current_sent_meter;
            ...
            <Table
                ...
                summary={() => {
                    ...
                    beamSentReport?.map((element) => {
                        ...
                        total_received_meter = total_received_meter + Number(element?.received_meter);
                        total_pending_meter = total_pending_meter + (Number(element?.sent_meter) - Number(element?.received_meter));
                    })
