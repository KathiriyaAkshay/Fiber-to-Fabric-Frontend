...
const YarnBillList = () => {
  ...
  const CalculateRateGst = (rate, net_amount, quantity) => {
    let rate_include_gst =
      Number(rate) + Number(Number(net_amount) / Number(quantity));
  ...
  const SummaryGeneration = async () => {
    ...
    res?.data?.data?.row?.map((element) => {
      ...
      if (temp_company.includes(supplier_company)) {
        ...
        element?.yarn_bill_details?.map((data) => {
          total_weight =
            total_weight + Number(data?.yarn_receive_challan?.receive_quantity);
        });
    ...
    let title = ["PARTY", "WEIGHT", "AMT", "CGST", "SGST", "IGST", "NET_AMT"];
