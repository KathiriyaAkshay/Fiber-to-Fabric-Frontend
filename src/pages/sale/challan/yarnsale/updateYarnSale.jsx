...
import { updateYarnSalerChallanRequest } from "../../../../api/requests/sale/challan/challan";
        ...
        if (name === yarn_company_name) {
          const options = yarn_details?.map(
            ({
              ...
              if (
                yarnSaleChallanDetails?.yarn_stock_company?.id ==
                yarn_company_id
              ) {
                // ==== Set Stock information ==== //

                let temp_current_stock =
                  current_stock + yarnSaleChallanDetails?.kg;
                setValue("current_stock", temp_current_stock);
                setCurrentStockInfo(temp_current_stock);

                let remain_stock =
                  Number(temp_current_stock) -
                  Number(yarnSaleChallanDetails?.kg);
  ...
  useEffect(() => {
    if (yarnSaleChallanDetails) {
      reset({
        ...
        yarn_company_name:
          yarnSaleChallanDetails?.yarn_stock_company?.yarn_company_name,
