import { Flex, Button, Modal, Table } from "antd";
import { useContext, useEffect, useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { GlobalContext } from "../../../contexts/GlobalContext";
import PieCharts from "./PieChart";
import { ArrowsAltOutlined } from "@ant-design/icons";
import { getDashboardPayableInformation } from "../../../api/requests/dashboard";
...
const DayPayableOutStanding = ({ setDayPayableData }) => {
  ...
  const { data: outStandingData, isPending } = useQuery({
    queryKey: ["dashbaord", "payable-outstanding-amount"],
    queryFn: async () => {
      const params = {
        company_id: companyId,
      };
      const response = await getDashboardPayableInformation({ params });
    ...
    if (sundryDebtorData) {
      ...
      chart_data.push({
        ...
        pv:
          temp_30_days?.reduce(
            (total, element) =>
              total + (+element?.bills?.final_net_amount || 0),
            0
          ) || 0,
      });
      chart_data.push({
      ...
      setCharData([
        {
          ...
          value: +parseFloat(100 - +dueAmount).toFixed(2),
        },
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "bill_no",
      render: (text, record) => {
        return <div>{record?.bills?.bill_no}</div>;
      },
    },
    {
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return (
          <div>{moment(record?.bills?.createdAt).format("DD-MM-YYYY")}</div>
        );
      },
    },
  ];
  ...
  return (
    <>
      <div className="chart-wrapper">
        ...
        <div style={{ width: "100%" }}>
          <PieCharts data={charData || []} />
        </div>
        <div className="dashboard-chart-card">
          <div className="dashboard-chart-header">
            <div className="dashboard-chart-title">Total Outstanding</div>
            <div className="dashboard-chart-total-dues">
              Total Dues :
              <span className="dues-value">
                <br />
                <span style={{ fontWeight: 400 }}>{final_net_amount}</span>
              </span>
            </div>
          </div>
          <div className="dashboard-chart-amount">{netAmount || 0}</div>
          <table className="dashboard-chart-details">
            <thead>
              <tr>
                <th>DueDays</th>
                <th>Bills</th>
          ...
          {outStandingData?.challan_result?.length > 0 ? (
            outStandingData.challan_result.map((element, index) => (
              <div className="challan-information-text" key={index}>
                <div
                  className="challan-information-text"
                  key={index}
                  onClick={() => {
                    ChallanClickHandler(element?.model);
                  }}
                >
                  {element?.model == "yarn_receive_challans"
                    ? "Total Yarn Receive Challan"
                    : element?.model == "purchase_taka_challan"
                    ? "Total Purchase Challan"
                    : element?.model == "job_taka_challan"
                    ? "Total Job Challan"
                    : element?.model == "job_rework_challan"
                    ? "Job Job Rework Challan"
                    : ""}
                  :
                  <span className="challan-information-count">
                    {element?.number_of_challans}
                  </span>
                </div>
              </div>
            ))
...
export default DayPayableOutStanding;
