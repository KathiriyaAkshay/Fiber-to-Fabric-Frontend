...
function UpdateSizeBeamDetail({
  ...
  const addNewRowHandler = () => {
    ...
    if (fields.length === selectedOrder.size_beam_order_details.length) {
      ...
      console.log("deleted record information");
      ...
      if (deletedRecords.length) {
        ...
        let remainingOrder = selectedOrder.size_beam_order_details.filter(
          (order) => {
            let temp = false;
        ...
        remainingOrder[0]["beam_no"] = `${appendBeamType}-${Number(appendLastBeamNumber) + 1}`
        remainingOrder[0]["is_append"] = true;
        setAppendLastBeamNumber((prev) => Number(prev) + 1) ; 
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  {...field}
                  placeholder="10"
                  readOnly={record?.is_append == true ? false : true}
                />
              )}
            />
          </Form.Item>
          ...
          >
            <Controller
              ...
              render={({ field }) => (
                <Input
                  {...field}
                  readOnly={record?.is_append ? false : true}
                  type="number"
                  min={0}
                  step={0.01}
                  placeholder="10"
