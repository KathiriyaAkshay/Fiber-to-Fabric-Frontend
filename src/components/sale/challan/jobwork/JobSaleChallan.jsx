...
import {
  createSaleJobWorkChallanBillRequest,
...
const JobWorkSaleChallanModel = ({
  ...
  const { data: jobWorkBillDetail = null } = useQuery({
    ...
    queryFn: async () => {
      if (MODE === "VIEW" || MODE === "UPDATE") {
        const res = await getSaleJobWorkChallanBillByIdRequest({
          id: details.id,
  ...
  const onSubmit = async (data) => {
    ...
    await generateJobWorkBill(newData);
  };
  ...
  useEffect(() => {
    if (jobWorkBillDetail) {
      setValue("E_way_bill_no", jobWorkBillDetail.E_way_bill_no);
      setValue("rate", jobWorkBillDetail.rate);
      setValue("amount", jobWorkBillDetail.amount);

      setValue("net_amount", jobWorkBillDetail.net_amount);
      setValue("net_rate", jobWorkBillDetail.net_rate);
      setValue("round_off", jobWorkBillDetail.round_off_amount);
      setValue("bill_date", dayjs(jobWorkBillDetail.bill_date));
      setValue("due_date", dayjs(jobWorkBillDetail.due_date));

      setValue("discount_amount", jobWorkBillDetail.discount_amount);
      setValue("discount_value", jobWorkBillDetail.discount_value);
      setValue("IGST_amount", jobWorkBillDetail.IGST_amount);
      setValue("IGST_value", jobWorkBillDetail.IGST_value);
      setValue("CGST_amount", jobWorkBillDetail.CGST_amount);
      setValue("CGST_value", jobWorkBillDetail.CGST_value);
      setValue("SGST_amount", jobWorkBillDetail.SGST_amount);
      setValue("SGST_value", jobWorkBillDetail.SGST_value);
    }
  }, [jobWorkBillDetail, setValue]);
