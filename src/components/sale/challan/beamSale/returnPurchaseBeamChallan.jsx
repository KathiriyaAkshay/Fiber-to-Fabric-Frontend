...
import { returnPuchaseBeamRequest } from "../../../../api/requests/purchase/purchaseSizeBeam";

const ReturnPurchaseBeamChallan = ({ details }) => {
    ...
    const handleSelectBeamChallan = (event, index) => {
        if (event.  target.checked) {
            setSelectedBeamId((prev) => {
                return [...prev, details?.recieve_size_beam_details[index]?.id]
            })
        } else {
            setSelectedBeamId((prev) => {
                return prev.filter(
                    (i) => i != details?.recieve_size_beam_details?.[index]?.id
                )
            })
        }
    }
    ...
    const { mutateAsync: returnBeamSaleChallan, isPending } = useMutation({
        mutationFn: async (data) => {
          const res = await returnPuchaseBeamRequest({
            data,
            params: {
    ...
    const handleReturn = async () => {
        if (selectedBeamId?.length == 0){
            ...
            let requestPayload = {
                ...
                "receive_size_beam_id": details?.id,
                "receive_size_beam_detail_ids": beam_sale_ids,
            ...
            >
                <Flex className="flex-col" ref={componentRef}>
                    ...
                    <div style={{ height: 400 }}>
                        ...
                        {details?.recieve_size_beam_details?.map((item, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <Col span={3} style={{color: isReturn?"red":"#000"}}>{item?.ends_or_tars || obj.tars || 0}</Col>
                                    <Col span={3} style={{color: isReturn?"red":"#000"}}>{item?.pano || 0}</Col>
                                    <Col span={4} style={{color: isReturn?"red":"#000"}}>{item?.taka || 0}</Col>
                                    <Col span={4} style={{color: isReturn?"red":"#000"}}>{item?.meters || obj.meter || 0}</Col>
                                    <Col span={4} style={{color: isReturn?"red":"#000"}}>{item?.net_weight || 0}</Col>
                                </Row>
                            );
                        })}
                    </div>
...
export default ReturnPurchaseBeamChallan;
