...
import { Modal, Form, Input, DatePicker, Button, Radio, Select, Table, Typography, Flex, message } from 'antd';
    ...
    return (
        ...
        >
            <Form form={form} layout="vertical">
                ...
                <Radio.Group defaultValue="fullPayment"
                    value={paymentOption}
                    onChange={(event) => {
                ...
                <Table
                    columns={paymentOption == "fullPayment"?fullPaymentColumns:partialPaymentColumns}
                    dataSource={data}
                    pagination={false}
                    style={{
                        marginTop: "20px", 
                    ...
                    summary={() => (
                        <>
                          <Table.Summary.Row key={"total"}>
                            ...
                            <Table.Summary.Cell>
                              <Text>
                                <span style={{ color: 'blue' }}>{totalAmount?.saleAmount || 0}</span>
                              </Text>
                            </Table.Summary.Cell>
                            {paymentOption !== "fullPayment" &&
                              Array(3)
                                .fill(null)
                                .map((_, index) => <Table.Summary.Cell key={`total-empty-${index}`} />)}
                          </Table.Summary.Row>
                          ...
                          <Table.Summary.Row key={"grandTotal"}>
                            ...
                            {paymentOption !== "fullPayment" &&
                              Array(3)
                                .fill(null)
                                .map((_, index) => <Table.Summary.Cell key={`grand-empty-${index}`} />)}
                          </Table.Summary.Row>
                        </>
                      )}
                ...
                <div style={{
                    ...
                    <Flex style={{
                        ...
                        <div style={{width :"48%"}}>
                            ...
                            <Input placeholder='Cheque number' value={chequeNumber} onChange={(event) => {setChequeNumber(event.target.value)}} />
                        </div>
                        ...
                        <div style={{width: "48%"}}>
                            ...
                            <DatePicker style={{
                                width: "100%"
                            }} disabledDate={disabledFutureDate} format="DD-MM-YYYY" value={chequeDate} onChange={(event) => {setChequeDate(event.target.value)}} />
                        </div>
                    </Flex>
                </div>
                ...
                <div style={{
                    ...
                    <Flex style={{
                        ...
                        <div style={{width: "48%"}}>
                            ...
                            <Input placeholder='PARTY Bank' value={partyBank} onChange={(event) => {setPartyBank(event.target.value)}} />
                        </div>
                        <div style={{width: "48%"}}>
                            <div className='sundary-bill-payment-label'>Bank</div>
                            <Select value={bankValue} onChange={(event) => {setBankValue(event)}} style={{
                                width: "100%"
                            }} placeholder = {"Bank Option"} options={bankOption}>
                            </Select>
                        </div>
                    </Flex>
                </div>

                <Radio.Group defaultValue="passbookUpdate" style={{
                    marginTop: 20
                }} value={updateOption} onChange={(event) => {setUpdateOption(event.target.value)}}>
